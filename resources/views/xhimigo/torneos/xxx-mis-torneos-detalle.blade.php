<div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 sm:w-auto">
    <div class="sm:flex sm:items-start" >
        ...
        <div class="mt-3 text-center sm:mt-0 sm:ml-2 w-full sm:text-left">
            ...
            <div class="mt-2"><p class="text-sm text-gray-500">{{ $data->nombre_2 }}</p></div>

            <div class="grid grid-cols-3 rounded-lg border border-gray-200 my-2 p-2 w-full">
              <div class="text-xs text-gary-400 border-b border-gray-200 mx-2"><span class="">Disciplina </span><br><span class="border-t border-gray-200">{{ $data->disciplina }}</span></div>
              <div class="text-xs text-gary-400 border-b border-gray-200 mx-2"><span class="">Modalidad </span><br><span class="">{{ $data->modalidad }}</span></div>
              <div class="text-xs text-gary-400 border-b border-gray-200 mx-2"><span class="">Categoria </span><br><span class="">{{ $data->categoria }}</span></div>

              <div class="text-xs text-gary-400"><span class="">Código </span><br><span class="">{{ $data->codigo }}</span></div>
              <div class="text-xs text-gary-400"><span class="">Inicio </span><br><span class="">{{ $data->fecha_inicio }}</span></div>
              <div class="text-xs text-gary-400"><span class="">Fin </span><br><span class="">{{ $data->fecha_fin }}</span></div>

              <div class="text-xs text-gary-400"><span class="">Status </span><br><span class="uppercase">{{ $data->status }}</span></div>
              <div class="text-xs text-gary-400"><span class="">Inscripciones </span><br><span class="uppercase">{{ $data->validado }}</span></div>
            </div>
